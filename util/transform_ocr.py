...
class resize_short_length(object):
    def __init__(self, short_length=520, fit_stride=8, return_padding=False, mean=None, ignore_label=255):
    ...
    def __call__(self, image, label=None):
        h, w = image.shape[:2]
        if h > w:
            new_h = self.short_length
            new_w = np.int(w * self.short_length / h + 0.5)
        else:
            new_w = self.short_length
            new_h = np.int(h * self.short_length / w + 0.5)        
        image = cv2.resize(image, (new_w, new_h),
                           interpolation=cv2.INTER_LINEAR)
        pad_w, pad_h = 0, 0
        if self.fit_stride is not None:
            pad_w = 0 if (new_w % self.fit_stride == 0) else self.fit_stride - (new_w % self.fit_stride)
            pad_h = 0 if (new_h % self.fit_stride == 0) else self.fit_stride - (new_h % self.fit_stride)
            image = cv2.copyMakeBorder(
                image, 0, pad_h, 0, pad_w, 
                cv2.BORDER_CONSTANT, value=tuple(x * 255 for x in self.mean)
            )
        ...
        new_image, new_label = np.ones((self.short_length, self.short_length, 3)), np.ones((self.short_length, self.short_length)) * self.ignore_label
        for i in range(3):
            new_image[:,:,i] = new_image[:,:,i] * 255 * self.mean[i]

        new_image[:image.shape[0], :image.shape[1], :] = image
        new_label[:label.shape[0], :label.shape[1]] = label
        return new_image.astype(np.uint8), new_label
...
class ScaleNormalize(object):
    ...
    def __call__(self, image_1, label_1, image_2, label_2):
        if self.std is None:
            for t, m in zip(image_1, self.mean):
                t.sub_(m)
        else:
            for t, m, s in zip(image_1, self.mean, self.std):
                t.sub_(m).div_(s)
        ########################################################
        if self.std is None:
            for t, m in zip(image_2, self.mean):
                t.sub_(m)
        else:
            for t, m, s in zip(image_2, self.mean, self.std):
                t.sub_(m).div_(s)
        return image_1, label_1, image_2, label_2

class ScaleToTensor(object):
    # Converts numpy.ndarray (H x W x C) to a torch.FloatTensor of shape (C x H x W).
    def __call__(self, image_1, label_1, image_2, label_2):
        if not isinstance(image_1, np.ndarray) or not isinstance(label_1, np.ndarray):
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray"
                                "[eg: data readed by cv2.imread()].\n"))
        if len(image_1.shape) > 3 or len(image_1.shape) < 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray with 3 dims or 2 dims.\n"))
        if len(image_1.shape) == 2:
            image_1 = np.expand_dims(image_1, axis=2)
        if not len(label_1.shape) == 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray labellabel with 2 dims.\n"))

        image_1 = torch.from_numpy(image_1.transpose((2, 0, 1)))
        if not isinstance(image_1, torch.FloatTensor):
            image_1 = image_1.float()
        label_1 = torch.from_numpy(label_1)
        if not isinstance(label_1, torch.LongTensor):
            label_1 = label_1.long()

        ############################################################################

        if not isinstance(image_2, np.ndarray) or not isinstance(label_2, np.ndarray):
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray"
                                "[eg: data readed by cv2.imread()].\n"))
        if len(image_2.shape) > 3 or len(image_2.shape) < 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray with 3 dims or 2 dims.\n"))
        if len(image_2.shape) == 2:
            image_2 = np.expand_dims(image_2, axis=2)
        if not len(label_2.shape) == 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray labellabel with 2 dims.\n"))

        image_2 = torch.from_numpy(image_2.transpose((2, 0, 1)))
        if not isinstance(image_2, torch.FloatTensor):
            image_2 = image_2.float()
        label_2 = torch.from_numpy(label_2)
        if not isinstance(label_2, torch.LongTensor):
...
class ScaleCrop(object):
    ...
    Args:
        ...
        int instead of sequence like (h, w), a square crop (size, size) is made.
    """
    def __init__(self, size, scale_range=[0.5, 0.75], crop_type='center', padding=None, ignore_label=255):
    ...
    def __call__(self, image, label):
        ...
        if self.crop_type == 'rand':
            ...
            h_off = int((h - self.crop_h) / 2)
            w_off = int((w - self.crop_w) / 2)
        image_1 = image[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        label_1 = label[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]

        ##############################################################################

        image = image_1.copy()
        label = label_1.copy()
        scale_ratio = self.scale_range[0] + random.random() * (self.scale_range[1] - self.scale_range[0])
        new_crop_h = int(self.crop_h * scale_ratio)
        new_crop_w = int(self.crop_w * scale_ratio)
        new_crop_h = (new_crop_h // 8 + 1) * 8
        new_crop_w = (new_crop_w // 8 + 1) * 8

        h, w = label.shape
        pad_h = max(new_crop_h - h, 0)
        pad_w = max(new_crop_w - w, 0)
        pad_h_half = int(pad_h / 2)
        pad_w_half = int(pad_w / 2)
        if pad_h > 0 or pad_w > 0:
            if self.padding is None:
        ...
        if self.crop_type == 'rand':
            ...
            h_off = int((h - new_crop_h) / 2)
            w_off = int((w - new_crop_w) / 2)
        image_2 = image[h_off:h_off+new_crop_h, w_off:w_off+new_crop_w]
        label_2 = label[h_off:h_off+new_crop_h, w_off:w_off+new_crop_w]
        image_2 = cv2.resize(image_2, (self.crop_w, self.crop_h), interpolation=cv2.INTER_LINEAR)
        label_2 = cv2.resize(label_2, (self.crop_w, self.crop_h), interpolation=cv2.INTER_NEAREST)
