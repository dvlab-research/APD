...
class Compose(object):
    ...
    def __call__(self, image, label, part):
        flag = 0
        for t in self.segtransform:
            image, label, part = t(image, label, part)
...
class ToTensor(object):
    ...
    def __call__(self, image, label, part):
        ...
        if not isinstance(part, torch.LongTensor):
            part = part.long()
        return image, label, part
...
class Resize(object):
    ...
    def __call__(self, image, label, part):
        ...
        part = cv2.resize(part, self.size[::-1], interpolation=cv2.INTER_NEAREST)
        return image, label, part
...
class RandScale(object):
    ...
    def __call__(self, image, label, part):
        ...
        part = cv2.resize(part, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return image, label, part
...
class Crop(object):
    ...
    def __call__(self, image, label, part):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            part = cv2.copyMakeBorder(part, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
        ...
        part = part[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        return image, label, part
...
class RandRotate(object):
    ...
    def __call__(self, image, label, part):
        if random.random() < self.p:
            ...
            part = cv2.warpAffine(part, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.ignore_label)
        return image, label, part
