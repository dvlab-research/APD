...
def print_model_parm_flops(fcn):
    fcn = add_flops_counting_methods(fcn)
    fcn = fcn.eval()
    from torch.autograd import Variable
    input = Variable(torch.rand(1, 3, 713, 713), requires_grad = True)
    fcn.start_flops_count()
    
    _ = fcn(input)
    
    print(fcn.compute_average_flops_cost() / 1e9 / 2) # Result in GFLOPs per image in batch
